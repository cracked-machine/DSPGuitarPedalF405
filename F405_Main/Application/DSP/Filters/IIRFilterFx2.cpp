...
IIRFilterFx2::IIRFilterFx2()
{
	...
	void IIRFilterFx2::process_half_u16(	AudioBlockU16< FULL_BLK_SIZE_U16 > *pRxBuf,
...
/*
		...
		AudioBlockU16< STEREO_CH_SIZE_U16 > pRxBufTmp;
		AudioBlockU16< STEREO_CH_SIZE_U16 > pTxBufTmp;
...
*/
		// transfer blocks (STEREO_CH_SIZE_U16) u16 samples
/*
		AudioBlockU16< STEREO_CH_SIZE_U16 > pRxBufTmp;
		AudioBlockU16< STEREO_CH_SIZE_U16 > pTxBufTmp;
...
*/
		...
		// copy first half sample block from rx in buffer into tmp rx buffer
		for(size_t i = 0; i < STEREO_CH_SIZE_F32*2; i++ ) {
	...
	void IIRFilterFx2::process_full_u16(	AudioBlockU16< FULL_BLK_SIZE_U16 > *pRxBuf,
											AudioBlockU16< FULL_BLK_SIZE_U16 > *pTxBuf)
	{
		if(pRxBuf == nullptr || pTxBuf == nullptr)
			error_handler();
...
/*		AudioBlockU16< STEREO_CH_SIZE_U16 > pRxBufTmp;
		...
		{
			pRxBufTmp[i-STEREO_CH_SIZE_U16] = (*pRxBuf)[i];
			pTxBufTmp[i-STEREO_CH_SIZE_U16] = pRxBufTmp[i-STEREO_CH_SIZE_U16];
			(*pTxBuf)[i] = pTxBufTmp[i-STEREO_CH_SIZE_U16];
		}
*/
/*
		// transfer blocks (STEREO_CH_SIZE_U16) u16 samples
...
/*
		// copy first half sample block from rx in buffer into tmp rx buffer
		for(size_t i = 0; i < MONO_CH_SIZE_F32; i++ ) {
			pLeftIn[i] = (float) ((*pRxBuf)[STEREO_CH_SIZE_U16 + i] << 16 | (*pRxBuf)[STEREO_CH_SIZE_U16 + i + 1]);
			pRightIn[i] = (float) ((*pRxBuf)[STEREO_CH_SIZE_U16 + i + 2] << 16 | (*pRxBuf)[STEREO_CH_SIZE_U16 + i + 3]);
		}
		// copy first half sample block into tmp tx buffer
		for(size_t i = 0; i < MONO_CH_SIZE_F32; i++ ) {
			pLeftOut[i] = pLeftIn[i];
		...
		for(size_t i = 0; i < MONO_CH_SIZE_F32; i++ )
		{
			(*pTxBuf)[STEREO_CH_SIZE_U16 + i] 	=  ( ((int)pLeftOut[i]) >> 16 ) & 0xFFFF;
			(*pTxBuf)[STEREO_CH_SIZE_U16 + i + 1] 	=  ( ((int)pLeftOut[i]) & 0xFFFF );
			(*pTxBuf)[STEREO_CH_SIZE_U16 + i + 2] 	=  ( ((int)pRightOut[i]) >> 16 ) & 0xFFFF;
			(*pTxBuf)[STEREO_CH_SIZE_U16 + i + 3] 	=  ( ((int)pRightOut[i]) & 0xFFFF );
		}
*/
	}

	void IIRFilterFx2::process_all_u16(	AudioBlockU16< FULL_BLK_SIZE_U16 > *pRxBuf,
										AudioBlockU16< FULL_BLK_SIZE_U16 > *pTxBuf)
	{
		if(pRxBuf == nullptr || pTxBuf == nullptr)
			error_handler();
